# Organize Felix and Valerie data

function Organize-Attachments($sourcePath, $destBase, $name) {
    Write-Host "`n========================================" -ForegroundColor Magenta
    Write-Host "  ORGANIZING $name DATA" -ForegroundColor Magenta
    ...
    if (-not $files) {
        Write-Host "No files found in $sourcePath" -ForegroundColor Yellow
        return
    }
    ...
    foreach ($file in $files) {
        ...
        if ($fname -match "bail|contrat.*service|acte.*vente|acte.*hypothe|convention.*achat|entente.*non.*concurr") {
        ...
        elseif ($fname -match "evaluation|analyse|fiche.*analyse|soumission.*plan|reglementaire|urbaniste|grille|usages|valorisation") {
        ...
        elseif ($fname -match "etat.*financ|ef.*20|bilan|flux.*tres|resultat") {
        ...
        elseif ($fname -match "contrat.*travail|lettre.*employ|offer.*letter|termination") {
    ...
    Write-Host "`nFichiers organises pour $name :" -ForegroundColor Cyan
...
# Organize Felix
Organize-Attachments "C:\Users\Utilisateur\jerome\FELIX-PIECES-JOINTES" "C:\Users\Utilisateur\jerome\FELIX-ORGANISE" "FELIX"

# Organize Valerie
Organize-Attachments "C:\Users\Utilisateur\jerome\VALERIE-PIECES-JOINTES" "C:\Users\Utilisateur\jerome\VALERIE-ORGANISE" "VALERIE"
